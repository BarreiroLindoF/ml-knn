...
def unit_circle_points(n):
    ...
    y_points = np.sqrt(1 - np.square(x_points))

    x_points_flipped = np.flip(x_points)
    y_points_flipped = np.multiply(y_points, -1)

    X[0] = np.concatenate((x_points, x_points_flipped))
    X[1] = np.concatenate((y_points, y_points_flipped))
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return X
